...
impl JsonRef {
    fn is_valid_pointer_fragment(pointer_str: &str) -> bool {
        pointer_str.is_empty() || pointer_str.starts_with('/')
    }

    fn parse_local_pointer(pointer_str: &str) -> Option<JsonPointer> {
        if !Self::is_valid_pointer_fragment(pointer_str) {
            return None;
        }
            ...
            ParseState::Invalid(inv) => {
                ...
                if inv.ownership == DiagnosticOwnership::Silent {
                    ctx.push_to_errors(
                        ...
                        format!("{}/{}", path, "$ref").into(),
                    );
                    reason = InvalidReason::InvalidReference;
                }
                // We will return invalid_emitted here because
                ParseState::invalid_emitted(reason)
            }
            // If the value of the "$ref" property cannot be parsed as a JsonRef, then it is an invalid format for a JsonRefObject, so we will return Invalid
            ParseState::NoMatch => ParseState::invalid_silent(InvalidReason::InvalidReference),
        }
    }
}
    ...
    fn try_from_json(
        ...
        match JsonRefObject::try_from_json(ctx, path, value) {
            ...
            ParseState::Invalid(inv) => {
                ...
                return ParseState::invalid_emitted(reason);
            }
            ParseState::NoMatch => {
                ...
                match T::try_from_json(ctx, path, value) {
                    ...
                    ParseState::Invalid(inv) => {
                        ...
                        if inv.ownership == DiagnosticOwnership::Silent {
                            ...
                            reason = InvalidReason::InvalidFieldType;
                        }
                        ParseState::invalid_emitted(reason)
                    }
                        ...
                        match T::try_from_json(ctx, path, value) {
                            ...
                            ParseState::Invalid(inv) => {
                                ...
                                if inv.ownership == DiagnosticOwnership::Silent {
                                    ...
                                    reason = InvalidReason::InvalidValue;
                                }
                                ParseState::invalid_emitted(reason)
                            }
                            ParseState::NoMatch => {
...
mod tests {
    ...
    fn test_json_ref_parse() {
        ...
        let root_fragment = JsonRef::parse("#").expect("Expected root fragment to parse");
        assert_eq!(root_fragment.document, None);
        assert_eq!(root_fragment.pointer, JsonPointer::new());

        let invalid_fragment = JsonRef::parse("#3366ff");
        assert!(
            invalid_fragment.is_none(),
            "Expected non-pointer fragment to fail parsing, but got Some"
        );

        let invalid_doc_fragment = JsonRef::parse("tokens.json#3366ff");
        assert!(
            invalid_doc_fragment.is_none(),
            "Expected document reference with non-pointer fragment to fail parsing, but got Some"
        );
