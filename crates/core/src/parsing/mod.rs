...
use crate::{
    DiagnosticProvenance, PROVENANCE_EXTENSION_KEY,
    errors::{
        DiagnosticCode, DiagnosticPhase, DiagnosticReporter, DiagnosticScope, ParseDiagnosticCode,
    },
    ir::{
        Deprecation, IrDocument, IrGroupToken, IrNode, IrTokenType, JsonPointer, JsonRef,
        ParseState, TokenAlias, TokenIdGenerator, TokenName,
    },
    parsing::token::{parse_common_fields, parse_raw_token_to_ir_token},
};

pub mod token;

pub struct ParserContext<'a> {
    diagnostics: DiagnosticScope<'a>,
...
impl<'a> ParserContext<'a> {
    pub fn new(file_content: &'a str, reporter: &'a mut DiagnosticReporter) -> Self {
        Self {
            diagnostics: DiagnosticScope::new(
                reporter,
                DiagnosticPhase::Parse,
                None,
                Some(JsonPointer::new()),
            ),
    ...
    pub fn diagnostics(&mut self) -> &mut DiagnosticScope<'a> {
    ...
    match raw_node {
        RawNode::Token(raw_token) => {
            ...
            match parse_raw_token_to_ir_token(raw_token, ctx, token_name, common) {
                ...
                ParseState::Failed(_) | ParseState::NoMatch => None,
            }
        }
                ...
                match IrTokenType::from_str(group_type) {
                    ...
                    None => ctx
                        ...
                        .error(DiagnosticCode::Parse(
                            ParseDiagnosticCode::InvalidTokenType {
                                found: group_type.clone(),
                            },
                        ))
                        .json_pointer(ctx.current_path)
                        .emit(),
                }
            }
...
fn extract_group_extension_reference<'a>(
    ...
    if let Some(extends_value) = current_map.get("$extends") {
        match extends_value.as_str() {
            ...
            None => {
                ctx.diagnostics()
                    .error(DiagnosticCode::Parse(
                        ParseDiagnosticCode::InvalidExtendsReference,
                    ))
                    .json_pointer(JsonPointer::from_segments(path))
                    .emit();
                return None;
            }
        }
    ...
    if let Some(ref_value) = current_map.get("$ref") {
        match ref_value.as_str() {
            ...
            None => {
                ctx.diagnostics()
                    .error(DiagnosticCode::Parse(
                        ParseDiagnosticCode::InvalidExtendsJSONReference,
                    ))
                    .json_pointer(JsonPointer::from_segments(path))
                    .emit();
                return None;
...
fn resolve_document_node<'a>(
    ...
    if extension_stack.contains(&path_key) {
        ctx.diagnostics()
            .error(DiagnosticCode::Parse(
                ParseDiagnosticCode::GroupCircularReference {
                    path: path_key.clone(),
                },
            ))
            .emit();
    ...
    let mut resolved_map = match extension_ref {
        Some(GroupReferenceDirective::Ref(ref_value)) => {
            let target_segments = match parse_extends_reference_segments(&ref_value) {
                ...
                Err(message) => {
                    let pointer = JsonPointer::from_segments(path);
                    ctx.diagnostics()
                        .error(DiagnosticCode::Parse(
                            ParseDiagnosticCode::InvalidExtendsJSONReference,
                        ))
            ...
            let target = match navigate_to_node(root, &target_segments) {
                ...
                None => {
                    ctx.diagnostics()
                        .error(DiagnosticCode::Parse(
                            ParseDiagnosticCode::UnresolvableJSONReference {
                                reference: ref_value.clone(),
                            },
                        ))
                        .json_pointer(JsonPointer::from_segments(path))
                        .help(format!("Unable to resolve JSON reference: '{}'", ref_value))
                        .emit();
                    extension_stack.pop();
                    return None;
                }
        ...
        | Some(GroupReferenceDirective::Extensions(ref_value)) => {
            ...
            let target_map = match target.as_object() {
                ...
                _ => {
                    ctx.diagnostics()
                        .error(DiagnosticCode::Parse(
                            ParseDiagnosticCode::InvalidGroupExtensionTarget {
                                reference: ref_value.clone(),
                            },
                        ))
                        .json_pointer(JsonPointer::from_segments(path))
...
pub fn parse_document<'a>(ctx: &mut ParserContext<'a>) -> Option<IrDocument> {
    let raw_value: serde_json::Value = match serde_json::from_str(ctx.file_content) {
        Ok(node) => node,
        Err(e) => {
            ctx.diagnostics()
                .error(DiagnosticCode::Parse(
                    ParseDiagnosticCode::JSONSerializationError {
                        message: e.to_string(),
                    },
                ))
                .emit();
        ...
        RawNode::Group(root_group) => {
            ...
            if let Some(group_type) = &root_group.token_type {
                match IrTokenType::from_str(group_type) {
                    Some(group_token_type) => ctx.set_current_type(group_token_type),
                    None => {
                        let pointer = ctx.current_path.clone();
                        ctx.diagnostics()
                            .error(DiagnosticCode::Parse(
                                ParseDiagnosticCode::InvalidTokenType {
                                    found: group_type.clone(),
...
mod tests {
    ...
    fn test_parse_document_builds_ir_tree() {
        ...
        let mut diagnostic_reporter = DiagnosticReporter::default();
        let mut ctx = ParserContext::new(&json_string, &mut diagnostic_reporter);
        let document = parse_document(&mut ctx);
        let diagnostic_report = diagnostic_reporter.finish();
    ...
    fn test_parse_document_reports_circular_group_extends() {
        ...
        assert!(diagnostic_report.diagnostics.iter().any(|error| error.code
            == DiagnosticCode::Parse(ParseDiagnosticCode::GroupCircularReference {
                path: "a".to_string()
            })));
    }
    ...
    fn test_parse_document_rejects_string_color_value() {
        ...
        // TODO: re-enable once diagnostics are mapped to provenance sources during parsing
        /* assert!(!diagnostic_report.diagnostics.is_empty());
        assert!(diagnostic_report.diagnostics.iter().any(|error| {
            error.json_pointer == Some(JsonPointer::from("/brand/primary/$value".to_string()))
    ...
    fn test_parse_document_reports_json_errors() {
        let json_string = "{ this is invalid json }".to_string();
        let mut diagnostic_reporter = DiagnosticReporter::default();
        let mut ctx = ParserContext::new(&json_string, &mut diagnostic_reporter);
    ...
    fn test_parse_document_maps_diagnostics_to_provenance_source() {
        ...
        // TODO: re-enable once diagnostics are mapped to provenance sources during parsing
        /* let mapped = diagnostic_report.diagnostics.iter().any(|error| {
