...
impl TokenGraph {
    ...
    /// Converts the graph into a DOT format string, which can be used for visualization with tools like Graphviz.
    ///
    /// The DOT format is a plain text graph description language that allows for the representation of nodes and edges in a way that can be easily visualized. This method iterates through the nodes and edges of the graph, constructing a string that represents the graph structure in DOT format.
    ///
    /// # Returns
    ///
    ...
    pub fn to_dot(&self) -> String {
        ...
        out.push_str("digraph TokenGraph {\n");
        out.push_str("  rankdir=LR;\n");
        out.push_str("  node [shape=box, fontname=\"Helvetica\", fontsize=10];\n");

        for index in self.ir_graph.node_indices() {
            if let Some(slot) = self.ir_graph.node_weight(index) {
                let label = format!(
                    "{}\\n{}\\n{}",
                    slot_label_owner(slot.owner),
                    slot_label_kind(slot.kind),
                    slot.local_pointer.to_string()
                );
                let _ = writeln!(
                    out,
                    "  {} [label=\"{}\"];",
        ...
        for edge in self.ir_graph.edge_references() {
            let _ = writeln!(
                out,
                "  {} -> {} [label=\"{}\"];",
                edge.source().index(),
                edge.target().index(),
                edge_label_kind(edge.weight().kind)
            );
        }
...
fn slot_label_owner(owner: OwnerId) -> String {
    match owner {
        OwnerId::Token(id) => format!("Token({})", id.0),
        OwnerId::Group(id) => format!("Group({})", id.0),
    }
}

fn slot_label_kind(kind: SlotKind) -> &'static str {
    match kind {
        SlotKind::TokenNode => "TokenNode",
        SlotKind::GroupNode => "GroupNode",
        SlotKind::Property => "Property",
    }
}

fn edge_label_kind(kind: EdgeKind) -> &'static str {
    match kind {
        EdgeKind::Alias => "Alias",
        EdgeKind::JsonRef => "JsonRef",
        EdgeKind::PropertyRef => "PropertyRef",
    }
}
...
mod tests {
    ...
    use crate::{ParserContext, graph::TokenGraph, parser::parse_document};
    ...
    fn builds_graph_with_color_property_ref_edges() {
        ...
        assert!(graph.to_dot().contains("components"));
    }
    ...
    fn builds_complex_graph_from_full_spectrum_fixture() {
        ...
        assert!(graph.count_edges_by_kind(crate::graph::EdgeKind::Alias) >= 1);
        assert!(graph.count_edges_by_kind(crate::graph::EdgeKind::JsonRef) >= 1);
        assert!(graph.count_edges_by_kind(crate::graph::EdgeKind::PropertyRef) >= 1);

        let dot = graph.to_dot();
        assert!(dot.contains("digraph TokenGraph"));
        assert!(dot.contains("Alias"));
        assert!(dot.contains("PropertyRef"));
